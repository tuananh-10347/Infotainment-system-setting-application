...
void Menu::compairKeyNumber(Setting* temp) //so sánh keyNumber
{
    ...
    if (per.size() > 0)
    {
        ...
        {
            if (per.getNumData(i)->getKeyNumber() == temp->getKeyNumber())
            {
                per.getNumData(i)->setName(temp->getName());
                per.getNumData(i)->setEmail(temp->getEmail());
                per.getNumData(i)->setOdoNumber(temp->getOdoNumber());
                per.getNumData(i)->setRemindService(temp->getRemindService());
                switch (thongbao)
                {
                ...
                case 1:
                    ...
                    per.getNumData(i)->getDisplay()->setLightLevel(temp->getDisplay()->getLightLevel());
                    per.getNumData(i)->getDisplay()->setTaploLight(temp->getDisplay()->getTaploLight());
                    per.getNumData(i)->getDisplay()->setScreenLight(temp->getDisplay()->getScreenLight());
                    break;
                case 2:
                    theodoi = 1;
                    //Kiểm tra KeyNumber, thiết lập lại thông tin
                    per.getNumData(i)->getSound()->setMedia(temp->getSound()->getMedia());
                    per.getNumData(i)->getSound()->setCall(temp->getSound()->getCall());
                    per.getNumData(i)->getSound()->setNavigation(temp->getSound()->getNavigation());
                    per.getNumData(i)->getSound()->setNotification(temp->getSound()->getNotification());
                    break;
                case 3:
                    theodoi = 1;        
                   //Kiểm tra KeyNumber, thiết lập lại thông tin
                    per.getNumData(i)->getGeneral()->setLanguage(temp->getGeneral()->getLanguage());
                    per.getNumData(i)->getGeneral()->setTimeZone(temp->getGeneral()->getTimeZone());
                    break;
                }
            }
        }
...
void Menu::sortName() //sap xep theo ten
{
    if (per.size() > 1)
    {
        for (size_t i = 0; i < per.size(); i++)
        {
            ...
            {
                if (per.getNumData(j)->getName().compare(per.getNumData(j+1)->getName()) == 1)
                {
                    per.Sort(j, j+1);
                }
...
void Menu::sortKeyNumber() //sắp xếp theo MSCN
{
    if (per.size() > 1)
    {
        for (size_t i = 0; i < per.size(); i++)
        {
            ...
            {
                if (stoi(per.getNumData(j)->getKeyNumber()) > stoi(per.getNumData(j+1)->getKeyNumber()))
                {
                    per.Sort(j, j+1);
                }
...
void Menu::showInforSound()
{
    for (size_t i = 0; i < per.size(); i++)
    {
        ...
        per.getNumData(i)->getSound()->xuatThongTin();
        cout << "----------------------------------------------------------------------------------";
        cout << "--------------------------------------------------------------------------------------" << endl;
    }
...
void Menu::showInforGeneral()
{
    for (size_t i = 0; i < per.size(); i++)
    {
        ...
        per.getNumData(i)->getGeneral()->xuatThongTin();
        cout << "----------------------------------------------------------------------------------";
        cout << "--------------------------------------------------------------------------------------" << endl;
    }
}
        ...
        if (settingOutput.is_open())
        {
            for (size_t i = 0; i < per.size(); i++)
            {
                settingOutput << "Common: "
                    << per.getNumData(i)->getName() << "," << per.getNumData(i)->getEmail() << ","
                    << per.getNumData(i)->getKeyNumber() << "," << per.getNumData(i)->getOdoNumber() << ","
                    << per.getNumData(i)->getRemindService() << " "
                    << "General: "
                    << per.getNumData(i)->getGeneral()->getLanguage() << ","
                    << per.getNumData(i)->getGeneral()->getTimeZone() << ";"
                    << "Sound: "
                    << per.getNumData(i)->getSound()->getMedia() << ","
                    << per.getNumData(i)->getSound()->getCall() << ","
                    << per.getNumData(i)->getSound()->getNavigation() << ","
                    << per.getNumData(i)->getSound()->getNotification() << ";"
                    << "Display: "
                    << per.getNumData(i)->getDisplay()->getLightLevel() << "," 
                    << per.getNumData(i)->getDisplay()->getScreenLight() << ","
                    << per.getNumData(i)->getDisplay()->getTaploLight() << "; " << endl;
            }
            settingOutput.close();
        }
        else 
